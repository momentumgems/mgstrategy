...
/* ===== Robust Export/Import (HTML file or Clipboard) ===== */
(function(){
  const exportBtn = document.getElementById("mg-export-html-btn");
  const importBtn = document.getElementById("mg-import-html-btn");
  const fileInput = document.getElementById("mg-import-file");
  const statusBox = document.getElementById("status-message");
  const statusText = document.getElementById("status-text");

  function showStatus(msg, isError=false, persist=false){
    if(!statusBox || !statusText) return alert(msg);
    statusText.textContent = msg;
    statusText.className = "px-4 py-2 rounded-md shadow-sm text-sm " + (isError?"bg-red-100 text-red-800":"bg-green-100 text-green-800");
  ...
  function serializeWithState(state){
    try{
      // Clone current document and inject state into #mg-seed
      const doc = document.cloneNode(true);
      const seed = doc.getElementById("mg-seed");
      if(seed){
        seed.textContent = JSON.stringify(state);
      } else {
  ...
  async function exportHTML(){
    try{
      if(typeof getState !== "function"){ throw new Error("App not ready"); }
      const current = getState();
      const html = serializeWithState(current);
      if(!html){ throw new Error("Could not build HTML snapshot."); }
      const blob = new Blob([html], {type:"text/html"});
      const a = document.createElement("a");
      a.href = URL.createObjectURL(blob);
      ...
      showStatus("Exported HTML with your latest data.");
    }catch(e){
      console.error(e);
      showStatus("Export failed: " + e.message, true, true);
    }
  }

  function parseSeedFromHTML(htmlText){
    try{
      const parser = new DOMParser();
      const doc = parser.parseFromString(htmlText, "text/html");
      const seed = doc.getElementById("mg-seed");
      if(!seed) throw new Error("No mg-seed found in file.");
      const json = JSON.parse(seed.textContent || "{}");
      return json;
    }catch(e){
      throw new Error("Invalid HTML or JSON: " + e.message);
    }
  }
  ...
  async function importFromFile(file){
    try{
      ...
      const data = parseSeedFromHTML(text);
      if(typeof saveState !== "function"){ throw new Error("App not ready"); }
      saveState(data);
      if(typeof router === "function") router();
      showStatus("Imported data from HTML file.");
    }catch(e){
      console.error(e);
      showStatus("Import failed: " + e.message, true, true);
    }
  }

  async function importFromClipboard(){
    try{
      if(!navigator.clipboard) throw new Error("Clipboard not available. Use file import instead.");
      const text = await navigator.clipboard.readText();
      if(!text || text.indexOf("<html") === -1) throw new Error("Clipboard does not contain exported HTML.");
      const data = parseSeedFromHTML(text);
      saveState(data);
      if(typeof router === "function") router();
      showStatus("Imported data from clipboard.");
    }catch(e){
      console.error(e);
      showStatus("Clipboard import failed: " + e.message, true, true);
    }
  }

  exportBtn && exportBtn.addEventListener("click", exportHTML);
  importBtn && importBtn.addEventListener("click", (e)=>{
    e.preventDefault();
    // Prefer file import via picker for reliability
    if(fileInput){
      fileInput.value = "";
      fileInput.click();
    } else {
